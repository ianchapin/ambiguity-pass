...
export function renderFriendly(a: Audit) {
  ...
  if (a.representation.short?.trim()) lines.push(K(a.representation.short.trim()));
  lines.push("");

  lines.push(H("1) What this is"));
  const kind = a.representation.kind?.trim() ?? "";
  lines.push(`  ${kind ? `${kind}: ` : ""}${trimOneLine(a.representation.raw)}`);
  lines.push("");

  lines.push(H("2) Intended use (attempted vs earned)"));
  lines.push(`  ${K("attempted use:")} ${a.intended_use.attempted_use}`);
  lines.push(`  ${K("earned tier:")} ${a.intended_use.earned_tier}`);
  if (a.intended_use.mismatch) {
    lines.push(chalk.yellow(`  mismatch: yes — ${a.intended_use.mismatch_reason}`));
  } else {
    lines.push(`  ${K("mismatch:")} no`);
  }
  if (a.intended_use.notes?.trim()) lines.push(`  ${K("notes:")} ${a.intended_use.notes.trim()}`);
  if (a.intended_use.alternatives?.length) {
    lines.push(`  ${K("also plausible:")} ${a.intended_use.alternatives.join(", ")}`);
  }
  lines.push("");
  ...
  lines.push(`  ${K("alternatives available:")} ${alts.length ? alts.join("; ") : "(none)"}`);
  if (a.decision_context.notes?.trim()) lines.push(`  ${K("notes:")} ${a.decision_context.notes.trim()}`);
  lines.push("");
  ...
  if (!a.ambiguities || a.ambiguities.length === 0) {
    ...
    for (const amb of a.ambiguities) {
      const pfx = amb.priority === "primary" ? chalk.cyan("primary") : chalk.gray("secondary");
      lines.push(`  ${pfx} ${chalk.cyan(amb.type)}: ${amb.description}`);
      lines.push(`  ${K("why it matters:")} ${amb.why_it_matters}`);
      if (amb.signals?.length) lines.push(`  ${K("signals:")} ${amb.signals.join("; ")}`);
      if (amb.what_to_do_now?.length) lines.push(`  ${K("do now:")} ${amb.what_to_do_now.join("; ")}`);
    }
  }
  lines.push("");

  lines.push(H("6) How much weight to put on it"));
  lines.push(`  ${chalk.magenta("Suggested reliance:")} ${a.confidence.reliance}`);
  lines.push(`  ${chalk.magenta("Max reliance allowed:")} ${a.confidence.reliance_cap}`);
  if (a.confidence.rationale?.trim()) lines.push(`  ${a.confidence.rationale.trim()}`);
  ...
  if (!a.failure_modes || a.failure_modes.length === 0) {
    ...
    for (const f of a.failure_modes) {
      ...
      if (f.mitigations?.length) lines.push(`    ${K("mitigate:")} ${f.mitigations.join("; ")}`);
      if (f.fallback?.length) lines.push(`    ${K("fallback:")} ${f.fallback.join("; ")}`);
    }
  }
  lines.push("");
  ...
  if (a.judgment_handoff.who_owns_it?.trim()) {
    lines.push(`  ${K("who owns it:")} ${a.judgment_handoff.who_owns_it.trim()}`);
  }
  lines.push(K("Next questions:"));
  lines.push(wrapBullets(a.judgment_handoff.next_questions));
  lines.push("");

  if (a.meta.gates_applied?.length) {
    lines.push(chalk.blue("Gates applied:"));
    lines.push(wrapBullets(a.meta.gates_applied));
    lines.push("");
  }
...
export function renderTechnical(a: Audit) {
  ...
  if (a.representation.kind?.trim()) lines.push(maybeLine("kind:", a.representation.kind).trimEnd());
  if (a.representation.short?.trim()) lines.push(maybeLine("short:", a.representation.short).trimEnd());
  lines.push("");

  lines.push(H("Intended use:"));
  lines.push(`- attempted_use: ${a.intended_use.attempted_use}`);
  lines.push(`- earned_tier: ${a.intended_use.earned_tier}`);
  lines.push(`- mismatch: ${a.intended_use.mismatch ? "true" : "false"}`);
  lines.push(`- mismatch_reason: ${a.intended_use.mismatch_reason?.trim() || "(none)"}`);
  lines.push(`- notes: ${a.intended_use.notes?.trim() || "(none)"}`);
  lines.push(`- alternatives:\n${wrapBullets(a.intended_use.alternatives ?? [])}`);
  lines.push("");
  ...
  lines.push(`- alternatives_available:\n${wrapBullets(a.decision_context.alternatives_available ?? [])}`);
  ...
  lines.push(`- who_owns_it: ${a.judgment_handoff.who_owns_it?.trim() || "(unknown)"}`);
  lines.push(`- next_questions:\n${wrapBullets(a.judgment_handoff.next_questions ?? [])}`);
  lines.push("");

  lines.push(K(`meta.generated_at_iso: ${a.meta.generated_at_iso}`));
  lines.push(K(`meta.model: ${a.meta.model}`));
  if (a.meta.schema_version) lines.push(K(`meta.schema_version: ${a.meta.schema_version}`));
  if (a.meta.gates_applied?.length) lines.push(K(`meta.gates_applied: ${a.meta.gates_applied.join("; ")}`));
  if (a.meta.warnings?.length) lines.push(K(`meta.warnings: ${a.meta.warnings.join("; ")}`));
